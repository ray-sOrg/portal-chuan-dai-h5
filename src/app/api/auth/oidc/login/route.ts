...
export async function GET(request: NextRequest) {
  ...
  const returnTo = requested?.startsWith("/") && !requested.startsWith("//") && !requested.includes("\\") ? requested : "/zh/home";
  ...
  response.cookies.set(OIDC_STATE_COOKIE + (silent ? "_silent" : ""), state, options);
  response.cookies.set(OIDC_ATTEMPT_COOKIE + (silent ? "_silent" : ""), `${codeVerifier}|${nonce}|${encodeURIComponent(returnTo)}`, options);
  return response;
}
