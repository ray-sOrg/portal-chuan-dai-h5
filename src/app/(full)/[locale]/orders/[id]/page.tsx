              ...
              <div key={item.id} className="p-4 flex justify-between items-start">
                <div className="flex-1">
                  ...
                  <div className="text-sm text-muted-foreground">
                    {item.weightGrams ? `${item.weightGrams.toNumber()}g · ¥${item.price.toNumber().toFixed(2)}/100g` : item.volumeMl ? `${item.volumeMl.toNumber()}ml · ¥${item.price.toNumber().toFixed(2)}/100ml` : `¥${item.price.toNumber().toFixed(2)} × ${item.quantity}`}
                  </div>
                  {item.remark && (
                ...
                <div className="font-medium">
                  ¥{item.price.times(item.weightGrams ? item.weightGrams.div(100) : item.volumeMl ? item.volumeMl.div(100) : item.quantity).toNumber().toFixed(2)}
                </div>
              </div>
            ))}
          </div>
